...
function AdminPanel({ services, barbers, reloadCatalog }) {
  ...
  const [barber, setBarber] = useState({ displayName: '', bio: '', photoUrl: '' }); const [editingBarber, setEditingBarber] = useState(null);
  ...
  const createBarber = async (event) => { event.preventDefault(); try { await api.createBarber({ ...barber, photoUrl: barber.photoUrl || null }); setBarber({ displayName: '', bio: '', photoUrl: '' }); await reloadCatalog(); saveMessage('Barbero registrado. Ahora puedes asignar sus servicios, horarios y cuenta.'); } catch (error) { saveMessage(error.message); } };
  const saveBarber = async (event) => { event.preventDefault(); try { await api.updateBarber(editingBarber.id, { displayName: editingBarber.displayName, bio: editingBarber.bio || undefined, photoUrl: editingBarber.photoUrl || null, isActive: editingBarber.isActive }); setEditingBarber(null); await reloadCatalog(); saveMessage('Información del barbero actualizada.'); } catch (error) { saveMessage(error.message); } };
