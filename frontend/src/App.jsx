...
function Landing({ servicesCount, barbersCount, settings, onReserve }) {
  const business = settings ?? { businessName: 'Barber Team Machine', address: 'Buenos Aires 130', city: 'Cochabamba', country: 'Bolivia', phone: '77469963', hoursText: 'Lunes a sábado · 10:00 AM — 7:00 PM', mapsUrl: 'https://maps.app.goo.gl/At4qrWe6P2KrDCyW9', description: 'Precisión, técnica y estilo para cada visita.' };
  useEffect(() => {
  ...
  return <>
    ...
    <section className="manifesto-section" id="nosotros"><div className="manifesto-title reveal-on-scroll"><p className="section-label">Nosotros</p><h2>NO ES SOLO<br/>UN CORTE.<br/><em>ES TU EXPERIENCIA.</em></h2></div><aside className="monogram-panel reveal-on-scroll"><span>BTM</span><p>{business.businessName}<br/>{business.city}, {business.country}<br/><br/>Corte · Barba · Cejas</p></aside><div className="manifesto-copy reveal-on-scroll"><p className="lead-copy"><strong>{business.businessName}</strong> es un espacio para hombres que entienden que cuidarse es una forma de mostrarse seguros.</p><p>{business.description}</p><p className="manifesto-quote">“Tu estilo habla antes que las palabras.”</p><div className="home-numbers"><div><b>{servicesCount || '—'}</b><span>Servicios</span></div><div><b>{barbersCount || '—'}</b><span>Barberos</span></div><div><b>6/7</b><span>Días abiertos</span></div><div><b>10AM</b><span>Apertura</span></div></div></div></section>
    ...
    <section className="location-section" id="ubicacion"><div className="reveal-on-scroll"><p className="section-label">Encuéntranos</p><h2>VISÍTANOS EN<br/><em>{business.city.toUpperCase()}.</em></h2></div><div className="location-content reveal-on-scroll"><iframe title={`Ubicación de ${business.businessName}`} src={`https://maps.google.com/maps?q=${encodeURIComponent(`${business.address}, ${business.city}, ${business.country}`)}&z=16&output=embed`} loading="lazy"/><div className="location-details"><article><b>Dirección</b><p>{business.address}<br/>{business.city}, {business.country}</p></article><article><b>Horario</b><p>{business.hoursText}</p></article><article><b>Contacto</b><p>+591 {business.phone}<br/><a href={business.mapsUrl} target="_blank" rel="noreferrer">Cómo llegar ↗</a></p></article></div><div className="location-actions"><a className="hero-cta" href={business.mapsUrl} target="_blank" rel="noreferrer">Cómo llegar <span>↗</span></a><button className="outline-link" onClick={onReserve}>Reservar cita <span>↗</span></button></div></div><p className="closing-line reveal-on-scroll">Tu próximo buen corte está a una cita de distancia.</p></section>
    <footer className="home-footer"><div className="reveal-on-scroll"><a className="brand-mark" href="#inicio">BTM<span>.</span></a><p>{business.description}</p></div><div className="reveal-on-scroll"><b>Navegación</b><a href="#nosotros">Nosotros</a><a href="#ubicacion">Ubicación</a><a href="#reservas">Reservar</a></div><div className="reveal-on-scroll"><b>Servicios</b><a href="#servicios">Corte normal</a><a href="#servicios">Perfilado de barba</a><a href="#servicios">Perfilado de cejas</a></div><div className="reveal-on-scroll"><b>Contacto</b><span>{business.address}</span><span>{business.city}, {business.country}</span><a href={`tel:+591${business.phone}`}>+591 {business.phone}</a></div><small className="reveal-on-scroll">© 2026 {business.businessName}. Todos los derechos reservados.</small></footer>
  </>;
}
...
function ReservationWizard() {
  ...
  const [comment, setComment] = useState(() => sessionStorage.getItem('btm_pending_client_notes') ?? '');
  ...
  function updateComment(value) {
    setComment(value);
    if (value.trim()) sessionStorage.setItem('btm_pending_client_notes', value);
    else sessionStorage.removeItem('btm_pending_client_notes');
  }
  ...
  async function reserve() {
    ...
    try {
      await api.createAppointments({ barberId: selection.barberId, serviceIds: selectedServiceIds, startsAt: selectedSlot.startsAt, clientNotes: comment.trim() || null });
      sessionStorage.removeItem('btm_pending_client_notes');
  ...
  return <main className="reservation-wizard">
    ...
    {step === 3 && <section className="checkout-stage">
      <div className="checkout-main">
        ...
        <label className="comment-box"><span>Tu sugerencia para la cita <small>Se guardará únicamente al confirmar la reserva.</small></span><textarea value={comment} onChange={(event) => updateComment(event.target.value)} placeholder="Escribe una solicitud opcional para tu barbero."/></label>
      </div>
      <aside className="checkout-summary">
        <h2>BTM · Cochabamba</h2>
        <p>📅 {selection.date && new Date(`${selection.date}T12:00:00`).toLocaleDateString('es-BO', { weekday: 'long', day: 'numeric', month: 'long' })}</p>
    ...
    {showProfessionalModal && <div className="professional-modal-backdrop" role="dialog" aria-modal="true" aria-label="Elegir profesional">
      <section className="professional-modal">
        ...
        <div className="professional-list">
          ...
          {filteredBarbers.map((barber) => <button key={barber.id} onClick={() => chooseProfessional(barber.id)}>{barber.photoUrl ? <img className="barber-avatar-photo" src={barber.photoUrl} alt=""/> : <b>{barber.displayName.slice(0, 1)}</b>}<strong>{barber.displayName}</strong><span>{barber.services || 'Barbero'}</span><i>★ Profesional BTM</i></button>)}
...
function BookingPage() {
  const [services, setServices] = useState([]); const [barbers, setBarbers] = useState([]); const [reviewsData, setReviewsData] = useState({ summary: { total: 0, average: 0, five: 0, four: 0, three: 0, two: 0, one: 0 }, reviews: [] }); const [settings, setSettings] = useState(null); const [session, setSession] = useState(() => JSON.parse(localStorage.getItem('btm_user') ?? 'null'));
  const [selection, setSelection] = useState({ serviceId: '', barberId: '', date: '' }); const [slots, setSlots] = useState([]); const [authMode, setAuthMode] = useState('login'); const [authForm, setAuthForm] = useState({ email: '', password: '', firstName: '', lastName: '' }); const [notice, setNotice] = useState(''); const [error, setError] = useState(''); const [serviceQuery, setServiceQuery] = useState(''); const [pendingClientNotes, setPendingClientNotes] = useState(() => sessionStorage.getItem('btm_pending_client_notes') ?? '');
  useEffect(() => { Promise.all([api.services(), api.barbers(), api.businessSettings(), api.reviews()]).then(([s, b, configuration, reviews]) => { setServices(s.services); setBarbers(b.barbers); setSettings(configuration.settings); setReviewsData(reviews); }).catch((e) => setError(e.message)); }, []);
  ...
  const business = settings ?? { businessName: 'Barber Team Machine', address: 'Buenos Aires 130', city: 'Cochabamba', country: 'Bolivia', phone: '77469963', hoursText: 'Lunes a sábado · 10:00 AM — 7:00 PM', mapsUrl: 'https://maps.app.goo.gl/At4qrWe6P2KrDCyW9' };
  const reviewSummary = reviewsData.summary ?? { total: 0, average: 0, five: 0, four: 0, three: 0, two: 0, one: 0 };
  const maxReviewsInTier = Math.max(1, Number(reviewSummary.five ?? 0), Number(reviewSummary.four ?? 0), Number(reviewSummary.three ?? 0), Number(reviewSummary.two ?? 0), Number(reviewSummary.one ?? 0));
  return <main className="booking-page booking-experience"><header className="booking-topbar"><a className="brand-mark" href="/">BTM<span>.</span></a><div>{session ? <button className="booking-login" onClick={() => { clearSession(); setSession(null); }}>Cerrar sesión</button> : <a className="booking-login" href="/reservar/cita">Iniciar sesión</a>}</div></header>
    <section className="booking-hero"><div className="booking-hero-mark">BTM<span>.</span></div><div className="booking-profile"><h1>{business.businessName}</h1><div><span>Barbería</span><span>{business.city}</span></div><p>★ Atención profesional <i>•</i> {business.address}, {business.city}</p><p>◷ Abierto: {business.hoursText}</p></div></section>
    <div className="booking-layout">{error && <p className="booking-notice error">{error}</p>}{notice && <p className="booking-notice success">{notice}</p>}<nav className="booking-tabs"><a href="#servicios">Servicios</a><a href="#equipo">Equipo</a><a href="/reservar/cita">Reservar</a></nav>
      <section className="booking-about"><div><h2>Sobre nosotros</h2><div className="booking-tags"><span>Barbería</span><span>Corte y estilo</span><span>Atención personalizada</span></div><p>Precisión, técnica y estilo para acompañarte en cada visita. Elige un servicio y reserva tu horario en línea.</p></div><aside><b>⌖ &nbsp; Dirección</b><p>{business.address}<br/>{business.city}, {business.country}</p><a href={business.mapsUrl} target="_blank" rel="noreferrer">Ver dirección ↗</a><b>◷ &nbsp; Horario</b><p>{business.hoursText}</p></aside></section>
      ...
      <section className="team-section showcase-team" id="equipo"><div className="community-heading"><div><p className="section-label">Nuestro equipo</p><h2>Colaboradores</h2></div><span>{barbers.length} profesional(es)</span></div><div className="showcase-team-row">{barbers.length ? barbers.map((barber) => <article key={barber.id}>{barber.photoUrl ? <img className="team-barber-photo" src={barber.photoUrl} alt={`Foto de ${barber.displayName}`}/> : <b>{barber.displayName.slice(0, 1)}</b>}<h3>{barber.displayName}</h3><p>{barber.services || 'Barbero'}</p><span>★ Equipo BTM</span></article>) : <p className="empty-state">El equipo estará disponible pronto.</p>}</div></section>
      <section className="booking-reviews" aria-labelledby="reviews-title"><div className="community-heading"><div><p className="section-label">Experiencias reales</p><h2 id="reviews-title">Reseñas</h2></div><span>{reviewSummary.total ? `${reviewSummary.total} reseña(s)` : 'Aún sin reseñas'}</span></div><div className="reviews-summary"><div className="reviews-score"><b>★</b><strong>{reviewSummary.total ? Number(reviewSummary.average).toFixed(1) : '—'}</strong><span>{reviewSummary.total ? `${reviewSummary.total} reseña(s)` : 'Las opiniones aparecerán aquí'}</span></div><div className="reviews-bars">{[5, 4, 3, 2, 1].map((rating) => { const total = Number(reviewSummary[{ 5: 'five', 4: 'four', 3: 'three', 2: 'two', 1: 'one' }[rating]] ?? 0); return <div key={rating}><span>{rating} ★</span><i><em style={{ width: `${(total / maxReviewsInTier) * 100}%` }}/></i><b>{total}</b></div>; })}</div></div>{reviewsData.reviews.length ? <div className="review-cards">{reviewsData.reviews.map((review) => <article key={review.id}><div className="review-avatar">{review.clientName.slice(0, 1)}</div><div><b>{review.clientName}</b><span>{'★'.repeat(review.rating)}{'☆'.repeat(5 - review.rating)}</span></div><p>{review.comment}</p><small>{new Date(review.createdAt).toLocaleDateString('es-BO', { day: 'numeric', month: 'long', year: 'numeric' })}{review.barberName ? ` · Con ${review.barberName}` : ''}</small></article>)}</div> : <div className="reviews-empty"><b>✦</b><div><strong>Las reseñas verificadas aparecerán aquí.</strong><p>Cuando un cliente califique una cita atendida, su opinión podrá mostrarse públicamente.</p></div></div>}</section>
      <section className="booking-note-section"><label className="booking-note-capture"><b>¿Tienes una sugerencia para tu cita?</b><span>Opcional · puedes escribirla sin iniciar sesión; se guardará al confirmar.</span><textarea value={pendingClientNotes} maxLength="1500" onChange={(event) => { const value = event.target.value; setPendingClientNotes(value); if (value.trim()) sessionStorage.setItem('btm_pending_client_notes', value); else sessionStorage.removeItem('btm_pending_client_notes'); }} placeholder="Por ejemplo: prefiero un corte conservador o una atención puntual."/></label></section>
...
function AdminPanel({ services, barbers, reloadCatalog }) {
  ...
  const [barber, setBarber] = useState({ displayName: '', bio: '' }); const [barberPhotoFile, setBarberPhotoFile] = useState(null); const [barberPhotoInputKey, setBarberPhotoInputKey] = useState(0); const [editingBarber, setEditingBarber] = useState(null); const [editingBarberPhotoFile, setEditingBarberPhotoFile] = useState(null);
  ...
  const [businessSettings, setBusinessSettings] = useState({ businessName: '', address: '', city: '', country: '', phone: '', whatsapp: '', hoursText: '', mapsUrl: '', instagramUrl: '', facebookUrl: '', description: '' });
  ...
  useEffect(() => { api.businessSettings().then((result) => result.settings && setBusinessSettings(result.settings)).catch((error) => setMessage(error.message)); }, []);
  ...
  const saveAdminNote = async (id) => { try { await api.updateAppointmentAdminNotes(id, adminNoteDrafts[id] ?? null); await refresh(); saveMessage('Nota interna guardada.'); } catch (error) { saveMessage(error.message); } };
  ...
  const createBarber = async (event) => { event.preventDefault(); try { const created = await api.createBarber(barber); if (barberPhotoFile) await api.uploadBarberPhoto(created.id, barberPhotoFile); setBarber({ displayName: '', bio: '' }); setBarberPhotoFile(null); setBarberPhotoInputKey((value) => value + 1); await reloadCatalog(); saveMessage('Barbero registrado y foto cargada correctamente. Ahora puedes asignar sus servicios, horarios y cuenta.'); } catch (error) { saveMessage(error.message); } };
  const saveBarber = async (event) => { event.preventDefault(); try { await api.updateBarber(editingBarber.id, { displayName: editingBarber.displayName, bio: editingBarber.bio || undefined, isActive: editingBarber.isActive }); if (editingBarberPhotoFile) await api.uploadBarberPhoto(editingBarber.id, editingBarberPhotoFile); setEditingBarber(null); setEditingBarberPhotoFile(null); await reloadCatalog(); saveMessage('Información del barbero actualizada.'); } catch (error) { saveMessage(error.message); } };
  ...
  const saveBusinessSettings = async (event) => { event.preventDefault(); try { const normalised = Object.fromEntries(Object.entries(businessSettings).map(([key, value]) => [key, value || null])); const result = await api.updateBusinessSettings(normalised); setBusinessSettings(result.settings); saveMessage('Configuración del negocio actualizada.'); } catch (error) { saveMessage(error.message); } };
  const exportReport = () => {
  ...
  return <section className="admin-section admin-workspace">
    ...
    <nav className="admin-tabs" aria-label="Secciones administrativas">{[['dashboard', 'Dashboard'], ['agenda', 'Agenda y citas'], ['servicios', 'Servicios'], ['equipo', 'Barberos y horarios'], ['clientes', 'Clientes'], ['reportes', 'Reportes'], ['configuracion', 'Configuración']].map(([id, label]) => <button key={id} className={activeTab === id ? 'active' : ''} onClick={() => setActiveTab(id)}>{label}</button>)}</nav>
    ...
    {activeTab === 'agenda' && <div className="admin-agenda-layout"><section className="admin-card"><div className="admin-card-heading"><div><h3>Agenda centralizada</h3><p>{appointments.length} cita(s) encontradas.</p></div></div><div className="admin-appointment-list">{appointments.length ? appointments.map((item) => <article key={item.id}><time><b>{new Date(item.startsAt).toLocaleDateString('es-BO', { day: '2-digit', month: 'short' })}</b><span>{new Date(item.startsAt).toLocaleTimeString('es-BO', { hour: '2-digit', minute: '2-digit' })}</span></time><div><strong>{item.clientName}</strong><p>{item.serviceName} · {item.barberName}</p>{item.clientNotes && <p className="appointment-note client"><b>Solicitud:</b> {item.clientNotes}</p>}</div><span className={`admin-status ${item.status.toLowerCase()}`}>{statusLabels[item.status]}</span><div className="admin-appointment-actions">{item.status === 'PENDING' && <button onClick={() => updateStatus(item.id, 'CONFIRMED')}>Confirmar</button>}{['PENDING', 'CONFIRMED'].includes(item.status) && <><button onClick={() => updateStatus(item.id, 'COMPLETED')}>Atendida</button><button onClick={() => updateStatus(item.id, 'NO_SHOW')}>No asistió</button><button className="danger" onClick={() => updateStatus(item.id, 'CANCELLED')}>Cancelar</button><button className="quiet" onClick={() => setReschedule({ id: item.id, startsAt: new Date(item.startsAt).toISOString().slice(0, 16) })}>Reprogramar</button></>}</div><div className="admin-note-editor"><label>Nota interna<textarea value={adminNoteDrafts[item.id] ?? item.adminNotes ?? ''} onChange={(event) => setAdminNoteDrafts({ ...adminNoteDrafts, [item.id]: event.target.value })} placeholder="Solo visible para administración y barbero."/></label><button className="quiet" onClick={() => saveAdminNote(item.id)}>Guardar nota</button></div></article>) : <p className="admin-empty">No hay citas con estos filtros.</p>}</div></section>{reschedule.id && <aside className="admin-card reschedule-card"><h3>Reprogramar cita</h3><p>La cita volverá al estado pendiente para que sea confirmada.</p><form onSubmit={reprogramAppointment}><input type="datetime-local" required value={reschedule.startsAt} onChange={(event) => setReschedule({ ...reschedule, startsAt: event.target.value })}/><button>Guardar nueva fecha</button><button type="button" className="quiet" onClick={() => setReschedule({ id: '', startsAt: '' })}>Cancelar</button></form></aside>}</div>}
    ...
    {activeTab === 'configuracion' && <section className="admin-card business-settings-card"><p className="section-label">Datos públicos</p><h3>Configuración de la barbería</h3><p>Estos datos se usan en las páginas públicas y en los correos de recordatorio.</p><form className="admin-form" onSubmit={saveBusinessSettings}><div><input placeholder="Nombre comercial" required value={businessSettings.businessName ?? ''} onChange={(event) => setBusinessSettings({ ...businessSettings, businessName: event.target.value })}/><input placeholder="Teléfono" required value={businessSettings.phone ?? ''} onChange={(event) => setBusinessSettings({ ...businessSettings, phone: event.target.value })}/></div><div><input placeholder="Dirección" required value={businessSettings.address ?? ''} onChange={(event) => setBusinessSettings({ ...businessSettings, address: event.target.value })}/><input placeholder="Ciudad" required value={businessSettings.city ?? ''} onChange={(event) => setBusinessSettings({ ...businessSettings, city: event.target.value })}/></div><div><input placeholder="País" required value={businessSettings.country ?? ''} onChange={(event) => setBusinessSettings({ ...businessSettings, country: event.target.value })}/><input placeholder="WhatsApp" required value={businessSettings.whatsapp ?? ''} onChange={(event) => setBusinessSettings({ ...businessSettings, whatsapp: event.target.value })}/></div><input placeholder="Horario general" required value={businessSettings.hoursText ?? ''} onChange={(event) => setBusinessSettings({ ...businessSettings, hoursText: event.target.value })}/><input type="url" placeholder="Enlace de Google Maps" value={businessSettings.mapsUrl ?? ''} onChange={(event) => setBusinessSettings({ ...businessSettings, mapsUrl: event.target.value })}/><input type="url" placeholder="Instagram (opcional)" value={businessSettings.instagramUrl ?? ''} onChange={(event) => setBusinessSettings({ ...businessSettings, instagramUrl: event.target.value })}/><input type="url" placeholder="Facebook (opcional)" value={businessSettings.facebookUrl ?? ''} onChange={(event) => setBusinessSettings({ ...businessSettings, facebookUrl: event.target.value })}/><textarea placeholder="Descripción corta" value={businessSettings.description ?? ''} onChange={(event) => setBusinessSettings({ ...businessSettings, description: event.target.value })}/><button>Guardar configuración</button></form></section>}
  </section>;
}
...
function BarberPortal() {
  ...
  async function markAsCompleted(id) {
    try { await api.completeBarberAppointment(id); setDashboard(await api.barberDashboard()); }
    catch (requestError) { setError(requestError.message); }
  }
  return <main className="barber-portal"><header><a href="/">BTM<span>.</span></a><div><span>{dashboard?.barber.displayName}</span><button onClick={() => { clearSession(); setSession(null); }}>Cerrar sesión</button></div></header><section className="barber-hero"><p className="section-label">Mi agenda</p><h1>Hola, {dashboard?.barber.displayName?.split(' ')[0]}.</h1><p>Estas son exclusivamente las citas asignadas a ti.</p><div><article><b>{today.length}</b><span>Citas de hoy</span></article><article><b>{upcoming.length}</b><span>Próximas citas</span></article><article><b>{dashboard?.services.length ?? 0}</b><span>Servicios asignados</span></article></div></section>{error && <p className="barber-error">{error}</p>}<section className="barber-dashboard"><div><h2>Agenda de hoy</h2>{today.length ? <div className="barber-appointments">{today.map((appointment) => <article key={appointment.id}><time>{new Date(appointment.startsAt).toLocaleTimeString('es-BO', { hour: '2-digit', minute: '2-digit' })}</time><div><b>{appointment.clientName}</b><span>{appointment.serviceName} · {appointment.durationMinutes} min</span>{appointment.clientNotes && <p className="barber-appointment-note"><b>Solicitud:</b> {appointment.clientNotes}</p>}{appointment.adminNotes && <p className="barber-appointment-note internal"><b>Nota del equipo:</b> {appointment.adminNotes}</p>}</div><i className={appointment.status.toLowerCase()}>{appointment.status}</i>{['PENDING', 'CONFIRMED'].includes(appointment.status) && <button className="barber-complete" onClick={() => markAsCompleted(appointment.id)}>Marcar atendida</button>}</article>)}</div> : <p className="barber-empty">No tienes citas para hoy.</p>}</div><aside><h2>Mi información</h2><b>Servicios</b><p>{dashboard?.services.map((service) => service.name).join(' · ') || 'Sin servicios asignados'}</p><b>Horario laboral</b><p>{dashboard?.workingHours.length ? 'Configurado por administración' : 'Sin horario configurado'}</p></aside></section><section className="barber-upcoming"><h2>Próximas citas</h2>{upcoming.length ? upcoming.map((appointment) => <article key={appointment.id}><div><b>{appointment.serviceName}</b><span>{appointment.clientName}</span>{appointment.clientNotes && <p className="barber-appointment-note"><b>Solicitud:</b> {appointment.clientNotes}</p>}{appointment.adminNotes && <p className="barber-appointment-note internal"><b>Nota del equipo:</b> {appointment.adminNotes}</p>}</div><time>{new Date(appointment.startsAt).toLocaleDateString('es-BO', { weekday: 'short', day: 'numeric', month: 'short' })} · {new Date(appointment.startsAt).toLocaleTimeString('es-BO', { hour: '2-digit', minute: '2-digit' })}</time><i>{appointment.status}</i>{['PENDING', 'CONFIRMED'].includes(appointment.status) && <button className="barber-complete" onClick={() => markAsCompleted(appointment.id)}>Atendida</button>}</article>) : <p className="barber-empty">No tienes próximas citas.</p>}</section></main>;
}

function AppointmentDetail({ appointment, statusLabel, onCancel, onAddToCalendar, settings }) {
  const business = settings ?? { businessName: 'Barber Team Machine', address: 'Buenos Aires 130', city: 'Cochabamba', country: 'Bolivia', mapsUrl: 'https://maps.app.goo.gl/At4qrWe6P2KrDCyW9' };
  ...
  return <aside className="appointment-detail">
    <div className="appointment-banner">{appointment.barberPhotoUrl ? <img className="appointment-barber-photo" src={appointment.barberPhotoUrl} alt={`Barbero ${appointment.barberName}`}/> : <span>BTM</span>}<b>BARBER TEAM<br/>MACHINE</b><small>{appointment.barberName}</small></div>
    <div className="appointment-detail-body">
      ...
      <div className="appointment-actions">
        ...
        <a href={business.mapsUrl} target="_blank" rel="noreferrer"><ActionIcon name="directions"/><span>Cómo llegar</span><i>›</i></a>
        <a href={business.mapsUrl} target="_blank" rel="noreferrer"><ActionIcon name="location"/><span>Ver ubicación</span><i>›</i></a>
      </div>
      <div className="appointment-summary-section"><h3>Resumen</h3><p><span>{appointment.serviceName}<small>{Math.round((endsAt - startsAt) / 60000)} min con {appointment.barberName}</small></span><b>Bs {Number(appointment.price ?? 0).toFixed(0)}</b></p><p className="appointment-total"><b>Total</b><b>Bs {Number(appointment.price ?? 0).toFixed(0)}</b></p>{appointment.clientNotes && <div className="appointment-client-note"><b>Tu solicitud</b><p>{appointment.clientNotes}</p></div>}</div>
      ...
      <div className="appointment-location"><h3>Cómo llegar</h3><iframe className="map-preview" title={`Ubicación de ${business.businessName}`} src={`https://maps.google.com/maps?q=${encodeURIComponent(`${business.address}, ${business.city}, ${business.country}`)}&z=16&output=embed`} loading="lazy"/><b>{business.address}, {business.city}, {business.country}</b><a href={business.mapsUrl} target="_blank" rel="noreferrer">Cómo llegar ↗</a></div>
...
function ClientArea() {
  ...
  useEffect(() => { api.businessSettings().then((result) => setSettings(result.settings)).catch((requestError) => setError(requestError.message)); }, []);
...
function App() {
  ...
  const [services, setServices] = useState([]); const [barbers, setBarbers] = useState([]); const [settings, setSettings] = useState(null); const [session, setSession] = useState(() => JSON.parse(localStorage.getItem('btm_user') ?? 'null'));
  ...
  const loadCatalog = async () => { const [s, b, configuration] = await Promise.all([api.services(), api.barbers(), api.businessSettings()]); setServices(s.services); setBarbers(b.barbers); setSettings(configuration.settings); };
  ...
  return <main className="app-shell"><Landing servicesCount={services.length} barbersCount={barbers.length} settings={settings} onReserve={goToBooking}/></main>;
}
